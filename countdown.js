/**
      GET CURRENT TIME, THEN BASED ON THE TIME SET
      THE DEADLINE FOR THE COUNTDOWN CLOCKS
**/
'use strict';
...
$('#user_info_form').validate({
    submitHandler: function submitHandler(form, e) {
        ...
        postUserData(userInfo, 'https://reprieve.bsd.net/page/s/executions---top-signup');
...
$('.facebook_share').click(function(e) {
    ...
    var shareImg = encodeURIComponent('https://s.bsd.net/reprieve/main/page/-/time-to-execution/images/noose_small.jpg');
    var hoursSpan = $('#facebook_countdown .hours span').html();
    var minutesSpan = $('#facebook_countdown .minutes span').html();
...
/**
      SLIDE 3
**/
$('#slide3_find_out').click(function(e) {
    e.preventDefault();
    nextSlide('.slide3');
});
...
/**
      SLIDE 5
**/
$('#slide5_next').click(function(e) {
    e.preventDefault();
    nextSlide('.slide5');

    $('.same_as_text').height($('.text_height').height());
});

/**
      JUMP TO ALI-AL SLIDE
**/
...
/**
      SLIDE 7/8 - PETITION SUBMIT
**/
$('#sign_petition_ali').click(function(e) {
    e.preventDefault();
    postUserData(userInfo, 'https://reprieve.bsd.net/page/s/executions---ali-al-nimr');
    $('.unsigned').hide();
    $('.signed').show();
    $('.petition').html('Ali');
    nextSlide('.slide8');
...
$('#sign_petition_andy').click(function(e) {
    e.preventDefault();
    postUserData(userInfo, 'https://reprieve.bsd.net/page/s/executions---andy-tsege');
    $('.unsigned').hide();
    $('.signed').show();
    $('.petition').html('Andy');
    nextSlide('.slide8');
...
/**
      RESIZES THE "SEE THROUGH" IMAGE ON SLIDE 6
      TO MATCH THE TEXT BOX ON THE LEFT
**/
...
var initializeClock = function initializeClock(id, endtime) {
    var clock = $('#' + id);
    var hoursSpan = $('#' + id + ' .hours span');
    var minutesSpan = $('#' + id + ' .minutes span');
    var secondsSpan = $('#' + id + ' .seconds span');

    function updateClock() {
        var t = getTimeRemaining(endtime);

        if (id === 'facebook_countdown') {
        ...
        $("meta[name='og:title']").attr("content", "An execution takes places somewhere in the world every six hours - and the next one is in " + ('0' + t.hours).slice(-2) + " hours and " + ('0' + t.minutes).slice(-2) + " minutes.");
    }
...
var postUserData = function postUserData(data, formUrl) {
    var formData = {
...
var nextSlide = function nextSlide(el) {
    var marginHeight = $('.top_margin').height();
    var nextSlide = $(el).closest('.slide').next();
    if (el === '.slide1') {
        $('.first_name').html(userInfo.firstname);
    }
    nextSlide.show();
...
var initSlider = function initSlider() {
    ...
    $("#slider").slider({
        ...
        slide: function slide(event, ui) {
            handle.html('<div class="value">' + numberWithCommas(ui.value) + '</div>');
        }
    });
};

/**
      SET THE USER INFO OBJECT UP,
      INIT ALL THE COUNTDOWN CLOCKS
**/
var userInfo = {};

$(function() {
    initializeClock('countdown', deadline);
    initializeClock('side_countdown', deadline);
    initializeClock('mobile_countdown', deadline);
    initializeClock('facebook_countdown', deadline);

    WebFont.load({
        google: {
            families: ['Open Sans:400,700,800', 'Merriweather:300,300i,400,400i,700,700i']
        }
    });
